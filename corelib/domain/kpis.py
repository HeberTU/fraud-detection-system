...
def card_precision_top_k_day(
    ...
    return df_day_top_k.tx_fraud.mean(), df_day.tx_fraud.sum() / 100


def card_precision_top_k(
    test_data: pd.DataFrame, top_k: int = 100
) -> Tuple[float, float]:
    ...
    for i in range(num_days):
        ...
        precision_top_k_value, max_performance = card_precision_top_k_day(
            df_day=df_day, top_k=top_k
        )

        precision_top_k_per_day_list.append(precision_top_k_value)
        perfect_precision_top_k_per_day_list.append(max_performance)
